import torch
from dataset_exp import HorseZebraDataset
import sys
...
def train_fn(
    ...
    for idx, (zebra, horse) in enumerate(loop):
        # データをGPUに移動
        zebra = zebra.to(config_exp.DEVICE)
        horse = horse.to(config_exp.DEVICE)
        ...
        with torch.cuda.amp.autocast():
            ...
            G_loss = (
                ...
                loss_G_H +                                          # 馬生成器の敵対的損失
                cycle_zebra_loss * config_exp.LAMBDA_CYCLE +            # シマウマのサイクル損失
                cycle_horse_loss * config_exp.LAMBDA_CYCLE +            # 馬のサイクル損失
                identity_horse_loss * config_exp.LAMBDA_IDENTITY +      # 馬のアイデンティティ損失
                identity_zebra_loss * config_exp.LAMBDA_IDENTITY        # シマウマのアイデンティティ損失
            )
...
def main():
    ...
    disc_H = Discriminator(in_channels=3).to(config_exp.DEVICE)  # 馬判別器
    disc_Z = Discriminator(in_channels=3).to(config_exp.DEVICE)  # シマウマ判別器
    gen_Z = Generator(img_channels=3, num_residuals=9).to(config_exp.DEVICE)  # 馬→シマウマ生成器
    gen_H = Generator(img_channels=3, num_residuals=9).to(config_exp.DEVICE)  # シマウマ→馬生成器
    ...
    # ========== チェックポイントの読み込み ==========
    # 学習を再開する場合
    if config_exp.LOAD_MODEL:
        load_checkpoint(config_exp.CHECKPOINT_GEN_H, gen_H, opt_gen, config_exp.LEARNING_RATE)
        load_checkpoint(config_exp.CHECKPOINT_GEN_Z, gen_Z, opt_gen, config_exp.LEARNING_RATE)
        load_checkpoint(config_exp.CHECKPOINT_CRITIC_H, disc_H, opt_disc, config_exp.LEARNING_RATE)
        load_checkpoint(config_exp.CHECKPOINT_CRITIC_Z, disc_Z, opt_disc, config_exp.LEARNING_RATE)

    # ========== データセットとデータローダーの作成 ==========
    # 学習用データセット
    dataset = HorseZebraDataset(
        root_horse=config_exp.TRAIN_DIR + "/horses",
        root_zebra=config_exp.TRAIN_DIR + "/zebras",
        transform=config_exp.transforms,
    )
    
    # データローダーの作成
    loader = DataLoader(
        dataset,
        batch_size=config_exp.BATCH_SIZE,      # 学習用バッチサイズ
        shuffle=True,                      # データをシャッフル
        num_workers=config_exp.NUM_WORKERS,    # 並列データ読み込み
    ...
    # ========== 学習ループ ==========
    for epoch in range(config_exp.NUM_EPOCHS):
        print(f"Epoch [{epoch+1}/{config_exp.NUM_EPOCHS}]")
        train_fn(
        ...
        # エポック終了時にモデルを保存
        if config_exp.SAVE_MODEL:
            save_checkpoint(gen_H, opt_gen, filename=config_exp.CHECKPOINT_GEN_H)
            save_checkpoint(gen_Z, opt_gen, filename=config_exp.CHECKPOINT_GEN_Z)
            save_checkpoint(disc_H, opt_disc, filename=config_exp.CHECKPOINT_CRITIC_H)
            save_checkpoint(disc_Z, opt_disc, filename=config_exp.CHECKPOINT_CRITIC_Z)
